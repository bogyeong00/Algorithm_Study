...
# 입력 받기
n, m = map(int, input().split())  # n: 회사의 개수, m: 도로의 개수
...
# 각 도로에 대한 정보를 입력받아 그래프에 반영
for _ in range(m):
    a, b = map(int, input().split())  # a와 b 사이에 도로가 존재함을 의미
    graph[a][b] = 1  # 양방향으로 이동 가능하므로 1로 설정
    graph[b][a] = 1

# 소개팅 상대(k)와 목적지(x) 입력 받기
